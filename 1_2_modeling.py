...
## Store Code Grouping for Stratified K-Fold Cross-Validation
cv_group = label['store_code'].values
...
def modelBuild(input_1, input_2, input_3, input_4, optm='adam', loss_wgt=[1, .2]):
    ...
    h4 = layers.Dense(16, activation='relu', name='B4_Dense1_3')(input4)
    h4 = layers.BatchNormalization(name='B4_Dense1_4')(h4)
    h4 = layers.Dense(4, activation='relu', name='B4_Dense1_4')(input4)
...
### Stratified 5-Fold Cross Validation (by Store Code) with shuffle
model_history = []
model_mse = []
model_r2 = []
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=2020)
for train, test in cv.split(X_1, cv_group, groups=cv_group):
    _model = modelBuild(X_1.shape[1:], X_2.shape[1:], X_3.shape[1:], 
                        X_4.shape[1:], optm=optimizer, loss_wgt=[1, 0.2])
    
    hist = _model.fit([X_1[train], X_2[train], X_3[train], X_4[train]], 
    ...
    y_true = y_main[test]
    y_pred = _model.predict([X_1[test], X_2[test], X_3[test], X_4[test]])[0]
...
plt.plot(model_history[3].get('Output_Main_loss')[50:], color='red', label='Loss')
plt.plot(model_history[3].get('val_Output_Main_loss')[50:], color='blue', label='Validation Loss')
plt.legend()
plt.show()

### Squared Error & R-Squared
fig, ax = plt.subplots(1, 2, figsize=(8,4))
plt.suptitle('Stratified 5-Fold Cross Validation : by Store Code')
ax[0].set_title('MSE of 5-Fold CV')
sns.boxplot(y=model_mse, ax=ax[0])
ax[1].set_title('R-Squared of 5-Fold CV')
sns.boxplot(y=model_r2, ax=ax[1])
plt.show()

print('MSE Mean : {}\nMSE : {}'.format(np.mean(model_mse),model_mse))
print('R2 Mean : {}\nR2 : {}'.format(np.mean(model_r2),model_r2))
